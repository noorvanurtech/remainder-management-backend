...
class AuthService {
  ...
  async register(data: Partial<IUser>): Promise<{ message: string }> {
    ...
    console.log(`[REGISTER] Created user ${user.email} with OTP: ${otpCode}`);

    // Emit event for new user registration
    appEventEmitter.emit(EVENTS.USER_REGISTERED, user.toObject());

    await this.sendOtp(user, otpCode, "login"); // Using 'login' context for verification
  ...
  async login(
    ...
    console.time("login-findByEmail");
    const user = await userService.findByEmail(email);
    console.timeEnd("login-findByEmail");

    if (!user) {
      throw new Error(MESSAGES.AUTH.USER_NOT_FOUND);
    }
    
    console.time("login-matchPassword");
    const isMatch = await user.matchPassword(password);
    console.timeEnd("login-matchPassword");
    if (!isMatch) {
    ...
    console.time("login-saveUser");
    await user.save();
    console.timeEnd("login-saveUser");

    console.log(`[LOGIN] Generated OTP for user ${email}: ${otpCode}`);

    await this.sendOtp(user, otpCode, "login");
